...
const Login = () => {
  ...
  const { login, signInWithGoogle, currentUser, userData } = useAuth();
            ...
            <motion.button
              ...
              onClick={async () => {
                try {
                  ...
                  // The user document is already created/fetched by AuthProvider's onAuthStateChanged
                  // We just need to determine the target path
                  const userRef = doc(db, "users", user.uid);
                  const userSnap = await getDoc(userRef);
                  ...
                  // Give the animation a moment to finish before navigating
                  setTimeout(() => navigate(targetPath), 1800);
                } catch (err) {
