...
export const Hero = () => {
    ...
    return (
        <section ref={ref} className="relative h-screen w-full overflow-hidden flex flex-col items-center justify-center text-center">
            ...
            {/* Content */}
            <motion.div style={{ opacity }} className="relative z-20 px-6 max-w-5xl mx-auto">

                {/* ── LOOP: breathing label ── */}
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.3 }} className="mb-6">
                    <BreathingLabel>Architecture & Interior Design</BreathingLabel>
                </motion.div>

                {/* ── LOOP: shimmer on hero gold text ── */}
                <motion.h1
                    variants={fadeUp}
                    initial="hidden"
                    animate="visible"
                ...
                >
                    Every Drawing<br />
                    <GoldShimmer className="text-[#d4af37] italic">Is a Promise.</GoldShimmer>
                </motion.h1>

                {/* ── LOOP: typewriter cycling taglines ── */}
                <motion.div
                    variants={fadeUp}
                    initial="hidden"
                    animate="visible"
                    custom={2}
                    className="text-white/60 text-lg md:text-xl font-light max-w-xl mx-auto mb-12 min-h-[2rem] leading-relaxed"
                >
                    <TypewriterLoop texts={[
                        'High-end design built on trust.',
                        'Where discipline meets beauty.',
                        'Spaces that reflect your soul.',
                        'Built on lasting results.',
                    ]} />
                </motion.div>
                ...
                >
                    <Link to="/portfolio"
                        className="group flex items-center gap-2 px-8 py-4 bg-[#d4af37] text-black font-semibold rounded-full transition-all duration-300 hover:bg-[#c49f27] hover:scale-105 shadow-[0_0_40px_rgba(212,175,55,0.25)]">
                        View Our Work
                        <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" />
                    </Link>
                    <Link to="/contact"
                        className="px-8 py-4 border border-white/20 text-white font-medium rounded-full backdrop-blur-sm hover:border-[#d4af37]/50 hover:bg-white/5 transition-all duration-300">
                        Start a Project
                    </Link>
                </motion.div>
            </motion.div>

            {/* ── LOOP: scroll indicator with pulsing ring ── */}
            <motion.div
            ...
            >
                <BreathingLabel className="text-[10px]">Scroll</BreathingLabel>
